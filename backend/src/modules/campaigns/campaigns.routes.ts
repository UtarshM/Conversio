...
router.get("/:id", requireSession, async (req, res, next) => {
  try {
    ...
    const result = await getCampaign(req.workspaceContext.workspaceId, id, prisma);
    res.json({ data: result });
  } catch (error) {
    next(error);
  }
...
router.post("/:id/launch", requireSession, async (req, res, next) => {
  try {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const result = await launchCampaign(req.workspaceContext.workspaceId, id, prisma);
    res.json({ data: result });
  } catch (error) {
    next(error);
  }
...
router.post("/:id/retry", requireSession, async (req, res, next) => {
  try {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const result = await retryCampaign(req.workspaceContext.workspaceId, id, prisma);
    res.json({ data: result });
  } catch (error) {
    next(error);
  }
