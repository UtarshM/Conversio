  ...
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        <div className="grid gap-4 lg:grid-cols-[1.15fr,0.85fr]">
          <section className="rounded-[1.5rem] border border-border bg-card p-5 shadow-card">
            ...
            <div className="mt-4 flex gap-2 flex-wrap">
              ...
              <div className="ml-auto flex items-center rounded-xl border border-input bg-background p-1 text-xs">
                <button
                  type="button"
                  onClick={() => setViewMode("kanban")}
                  className={`px-3 py-1.5 rounded-lg font-medium transition-colors ${viewMode === "kanban" ? "bg-primary text-primary-foreground font-semibold" : "text-muted-foreground hover:text-foreground"}`}
                >
                  Kanban Board
                </button>
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg font-medium transition-colors ${viewMode === "list" ? "bg-primary text-primary-foreground font-semibold" : "text-muted-foreground hover:text-foreground"}`}
                >
                  List View
                </button>
              </div>
        ...
        {viewMode === "kanban" ? (
          <section className="rounded-[1.5rem] border border-border bg-card p-6 shadow-card overflow-x-auto">
            <h3 className="text-base font-semibold text-foreground mb-4">Kanban Sales Pipeline</h3>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 min-w-[900px]">
              {(["New", "Contacted", "Qualified", "Won", "Lost"] as const).map((stage) => {
                ...
                return (
                  <div key={stage} className="rounded-2xl border border-border bg-muted/20 p-4 flex flex-col min-h-[350px]">
                    <div className="flex items-center justify-between pb-3 border-b border-border mb-3">
                      <span className="text-xs font-bold uppercase tracking-wider text-foreground">{stage}</span>
                      <span className="h-5 px-2 rounded-full bg-primary/10 text-primary text-xs font-semibold flex items-center justify-center">
                        {stageLeads.length}
                      </span>
                    </div>
                    <div className="space-y-3 flex-1 overflow-y-auto">
                      {stageLeads.map((lead) => (
                        <div
                          ...
                          className={`p-3 rounded-xl border bg-card hover:shadow-md transition-all cursor-pointer ${activeLead?.id === lead.id ? "border-primary ring-1 ring-primary/30" : "border-border"}`}
                        >
                          <p className="text-sm font-semibold text-foreground line-clamp-1">{lead.fullName}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">{lead.phone}</p>
                          <span className="mt-2 inline-block px-2 py-0.5 text-[10px] font-mono rounded bg-muted text-muted-foreground">
                            {lead.source}
                          </span>
                          <div className="mt-3 flex items-center justify-between pt-2 border-t border-border/50 text-[11px]">
                            <span className="text-muted-foreground truncate">{lead.assignedTo || "Unassigned"}</span>
                            <select
                              value={lead.status}
                              onChange={(e) => {
                                e.stopPropagation();
                                handleLeadUpdate({ id: lead.id, status: e.target.value as any }, `Lead moved to ${e.target.value}`);
                              }}
                              className="h-6 text-[10px] rounded border border-input bg-background px-1 text-foreground"
                            >
                      ...
                      {stageLeads.length === 0 && (
                        <div className="h-24 rounded-xl border border-dashed border-border/60 flex items-center justify-center text-xs text-muted-foreground font-mono">
                          No leads
                        </div>
                      )}
                    </div>
