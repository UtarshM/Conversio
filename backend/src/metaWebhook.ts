...
export async function persistWhatsAppWebhookEvent(
  ...
  for (const message of event.inboundMessages) {
    ...
    // If customer clicked an interactive button (COD confirmation, prepaid discount, or NDR reattempt)
    if (message.interactiveId || message.interactiveTitle || message.body) {
      try {
        const { handleWhatsAppInteractiveReply } = await import("./modules/commerce/recovery.service");
        await handleWhatsAppInteractiveReply(
          workspaceId,
          message.from,
          message.interactiveId || "",
          message.interactiveTitle || message.body || ""
        );
      } catch (err) {
        console.error("[webhook] error handling interactive commerce button", err);
      }
    }
  }
