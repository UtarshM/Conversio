...
export const mockApi = {
  ...
  async updateBranding(input: Partial<Branding>): Promise<Partner> {
    const partners = readPartners();
    ...
    const updated: Partner = {
      ...
      customDomain: input.customDomain ?? current.customDomain,
      privacyPolicyUrl: input.privacyPolicyUrl ?? current.privacyPolicyUrl,
      termsUrl: input.termsUrl ?? current.termsUrl,
      conversationMarkup: input.conversationMarkup ?? current.conversationMarkup,
    };

    writePartners([updated, ...partners.filter((p) => p.id !== updated.id)]);
    return updated;
  },

  async verifyCustomDomain(domain: string): Promise<any> {
    return {
      verified: true,
      domain,
      targetCname: "app.conversio.ai",
      partner: readPartners()[0],
    };
  },

  async getPartnerSubWorkspaces(): Promise<any[]> {
    return [
      {
        id: "ws_client_1",
        name: "Acme D2C Store",
        plan: "growth",
        createdAt: new Date().toISOString(),
        users: [{ id: "u1", name: "Rahul Sharma", email: "rahul@acmed2c.in", role: "ADMIN" }],
        whatsAppConnections: [{ id: "wa1", display_phone_number: "+91 98765 43210", verified_name: "Acme Store", status: "connected" }],
      },
      {
        id: "ws_client_2",
        ...
        plan: "starter",
        createdAt: new Date().toISOString(),
        users: [{ id: "u2", name: "Vikram Mehta", email: "vikram@primerealty.com", role: "ADMIN" }],
        whatsAppConnections: [{ id: "wa2", display_phone_number: "+91 99887 76655", verified_name: "Prime Estates", status: "connected" }],
      },
    ];
  },

  async createPartnerSubWorkspace(input: any): Promise<any> {
    return {
      workspace: {
        id: `ws_${Date.now()}`,
        name: input.name,
  ...
  async topupPartnerSubWorkspace(_subWorkspaceId: string, _input: any): Promise<any> {
    return { success: true };
  },

  async getPartnerCommissions(): Promise<any[]> {
    return [
      {
        id: "comm_1",
        partnerId: "partner_demo",
        workspaceId: "ws_parent",
        sourceWorkspaceId: "ws_client_1",
        amount: 2000,
        revenueAmount: 10000,
        commissionRate: 20,
  ...
  async getCurrentBranding(): Promise<Branding> {
    return {
      brandName: "Conversio",
      logoUrl: "/favicon.svg",
...
export interface AppApi {
  ...
  getPartnerDashboard: () => Promise<any>;
  ...
  updateBranding: (input: Partial<Branding>) => Promise<Partner>;
  verifyCustomDomain: (domain: string) => Promise<any>;
  getPartnerSubWorkspaces: () => Promise<any[]>;
  createPartnerSubWorkspace: (input: any) => Promise<any>;
  topupPartnerSubWorkspace: (subWorkspaceId: string, input: any) => Promise<any>;
  getPartnerCommissions: () => Promise<any[]>;
  getCurrentBranding: () => Promise<Branding>;
  // Admin & Branding
