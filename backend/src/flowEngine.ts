...
/**
 * Match inbound message text against keyword automation rules.
 */
export function matchKeywordTrigger(
  ...
  const cleanBody = incomingBody.trim().toLowerCase();
  ...
  for (const rule of rules) {
    ...
    let isMatch = false;
    if (type === "exact") {
      isMatch = cleanBody === keyword;
    } else if (type === "startswith") {
      isMatch = cleanBody.startsWith(keyword);
    } else if (type === "regex") {
      try {
        isMatch = new RegExp(keyword, "i").test(cleanBody);
      } catch {
        isMatch = false;
      }
    } else {
      // "contains" default
      isMatch = cleanBody.includes(keyword);
    }
    ...
    if (isMatch) {
      return {
        ...
        matchType: type,
        autoReplyText: rule.replyText,
        assignToAgent: rule.assignTo,
        addTags: rule.tags,
      };
    }
  }
